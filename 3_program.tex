...
We focus on a language that only allow integer data type and hence the input is a data word. It is allowed to iterate though the input data word only once. Later in Section~\ref{sec:cases} we will show an extension that allows the reset of data word iterators.
...
Give a program $p$ and a data word $w$, we use $p(w)$ to denote the output of $p$ w.r.t. $w$. Formal semantics of the language is given in the appendix.
