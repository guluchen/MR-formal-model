...
\section{Analysis of a lasso}

Let $\Ss=(Q, X, Y, \delta, q_0, O)$ be a SDSIT$_{\pm}$. Suppose $Y=Y_1 \cup Y_2$, $Y_1 \cap Y_2 = \emptyset$, $Y_1=\{y_{1,1}, \dots, y_{1,l_1}\}$ and $Y_2 = \{y_{2,1}, \dots, y_{2,l_2}\}$. We go step by step.
...
Then for each $j: 1 \le j \le l_1$, an expression $e_{1,j}=\alpha^1_{j,0}+ \alpha^1_{j,1}d_1 + \dots \alpha^1_{j,n}d_n$ can be constructed to describe the value of $y_{1,j}$ after going through the handle, where the coefficients $\alpha^1_{j,0},\dots, \alpha^1_{j,n}$ are obtained from the transitions in the handle. Note here we ignore the special situations that the value of $y_{1,j}$ is undefined after going through the handle. Similarly, for each $j: 1 \le j \le l_2$, an expression $e_{2,j}=\alpha^2_{j,0}+ \alpha^2_{j,1}d_1 + \dots \alpha^2_{j,n}d_n$ can be constructed to describe the value of $y_{2,j}$ after going through the handle.

Let $\theta$ be the assignment function such that $\theta(y_{1,j})=e_{1,j}$ for each $j: 1 \le j \le l_1$ and $\theta(y_{2,j})=e_{2,j}$ for $j: 1 \le j \le l_2$.

Suppose $O(q_n)=a_0+a^1_1 y_{1,1} + \dots + a^1_{l_1} y_{1,l_1} + a^2_1 y_{2,1} + \dots + a^2_{l_2} y_{2,l_2}$. Then $\theta(O(q_n))$, that is, the expression obtained by replacing each $y_{1,j}$ with $e_{1,j}$ and $y_{2,j}$ with $e_{2,j}$ in $O(q_n)$, is $a^1_0+a^1_1 e_{1,1} + \dots + a^1_{l_1} e_{1,l_1} + a^2_1 e_{2,1} + \dots + a^2_{l_2} e_{2,l_2}$, which can be rearranged into the following expression,
\[
(a_0 + \sum \limits_{b=1,2, 1 \le j \le l_b} \alpha^b_{j,0}) + (\sum\limits_{b=1,2, 1 \le j \le l_b} a^b_j \alpha^b_{j,1}) d_1 + \dots + (\sum \limits_{b=1,2, 1 \le j \le l_b} a^b_j \alpha^b_{j,n}) d_n.
\]
Let $\mu_0,\dots,\mu_n$ denote the coefficients of the expression. 
If $\mu_i \neq 0$ for some $i: 1 \le i \le n$, then we know that there is an input $w$ such that the output of $\Ss$ on $w$ is nonzero.
...
Similarly to the handle, for each $b=1,2, j: 1 \le j \le l_b$, an expression $f_{b,j} = \beta^b_{j,0} + \beta^b_{j,1} \theta(y_{b,1}) + \dots + \beta^b_{j,l_b} \theta(y_{b,l_b}) + \gamma^b_{j,1} d'_1 + \dots + \gamma^b_{j,m} d'_m$ can be constructed to describe the value of the variable $y_{b,j}$ after going through the loop, where $\theta(y_{b,1}), \dots, \theta(y_{b,l_b})$ denote the initial value of the output variables and $d'_1, \dots, d'_m$ denote the data values on the loop.

Let $\chi$ be the assignment function such that $\chi(y_{b,j})=f_{b,j}$ for each $b=1,2$ and $j: 1 \le j \le l_b$.

Then $\chi(O(q_n)) = a_0 + a^1_{1} f_{1,1} + \dots + a^1_{l_1} f_{1,l_1}+a^2_{1} f_{2,1} + \dots + a^2_{l_2} f_{2,l_2}$ can be rearranged into the following expression,
\[
\begin{array} {l}
(a_0 + \sum \limits_{b'=1,2, 1 \le j' \le l_{b'}} \beta^{b'}_{j',0} ) + (\sum \limits_{b'=1,2} (\sum\limits_{1 \le j' \le l_{b'}} a^{b'}_{j'} \beta^{b'}_{j',1} ) \theta(y_{b',1})) + \dots + (\sum \limits_{b'=1,2} (\sum \limits_{1 \le j' \le l_{b'}} a^{b'}_{j'} \beta^{b'}_{j',l_{b'}}) \theta(y_{b',l_{b'}})) \ + \\
 (\sum\limits_{b'=1,2} (\sum \limits_{1 \le j' \le l_{b'}} a^{b'}_{j'} \gamma^{b'}_{j',1} )) d'_1 + \dots + (\sum \limits_{b'=1,2} (\sum \limits_{1 \le j' \le l_{b'}} a^{b'}_{j'} \gamma^{b'}_{j',m} )) d'_m.
\end{array}
\]


If $(\sum\limits_{b'=1,2}  (\sum \limits_{1 \le j' \le l_{b'}} a^{b'}_{j'} \gamma^{b'}_{j',i})) \neq 0$ for some $i: 1 \le i \le m$, then we are done.


Otherwise, let $a'_0, (a^1_1)',\dots, (a^1_{l_1})', (a^2_1)',\dots, (a^2_{l_2})'$ be the coefficients in the expression above, that is, $\chi(O(q_n))$ becomes $a'_0 + (a^1_1)' \theta(y_{1,1}) + \dots + (a^1_{l_1})' \theta(y_{1,l_1}) + (a^2_1)' \theta(y_{2,1}) + \dots (a^2_{l_2})' \theta(y_{2,l_2})$.

By expanding the expressions $\theta(y_{b,j})=e_{b,j}$ in $\chi(O(q_n))$, we get the following expression,

\[
(a'_0 + \sum \limits_{b=1,2, 1 \le j \le l_b} \alpha^b_{j,0}) + (\sum\limits_{b=1,2, 1 \le j \le l_b} (a^b_j)' \alpha^b_{j,1}) d_1 + \dots + (\sum \limits_{b=1,2, 1 \le j \le l_b} (a^b_j)' \alpha^b_{j,n}) d_n.
\]

%\[
%\begin{array}{l}
%((a_0 + \sum \limits_{b'=1,2, 1 \le j' \le l_{b'}} (c^{b'}_{j',0})' ) + \sum \limits_{b=1,2, 1 \le j \le l_b} c^b_{j,0}) \ +\\
% (\sum\limits_{b=1,2, 1 \le j \le l_b} (\sum\limits_{b'=1,2, 1 \le j' \le l_{b'}} a^{b'}_{j'} (c^{b'}_{j',j})' ) c^b_{j,1})\ d_1 + \dots \\
%+ (\sum\limits_{b=1,2, 1 \le j \le l_b} (\sum\limits_{b'=1,2, 1 \le j' \le l_{b'}} a^{b'}_{j'} (c^{b'}_{j',j})' ) c^b_{j,n})\ d_n.
%\end{array}
%\]
For each $i: 0 \le i \le n$, let $\mu'_i$ denote the $i$-th coefficient of the expression. 
%
...
In order to see this, let us iterate once more of the loop. Then we get the following output,
\[a'_0 + (a^1_1)' \chi(y_{1,1}) + \dots + (a^1_{l_1})' \chi(y_{1,l_1}) + (a^2_1)' \chi(y_{2,1}) + \dots (a^2_{l_2})' \chi(y_{2,l_2}).\]

By expanding the expressions $\chi(y_{b,j})$, we get the following expression,

\[
(a'_0 + \sum \limits_{b=1,2, 1 \le j \le l_b} (\alpha^b_{j,0})') + (\sum\limits_{b=1,2, 1 \le j \le l_b} (a^b_j)' (\alpha^b_{j,1})' ) d_1 + \dots + (\sum \limits_{b=1,2, 1 \le j \le l_b} (a^b_j)' (\alpha^b_{j,n})') d_n,
\]
where the coefficients $(\alpha^b_{j,0})', (\alpha^b_{j,1})', \dots, (\alpha^b_{j,n})'$ satisfy that 
$f_{b,j} = (\alpha^b_{j,0})' + (\alpha^b_{j,1})' d_1 + \dots + (\alpha^b_{j,n})' d_n$, more specifically, 
\[
f_{b,j} = (\beta^b_{j,0} + \sum \limits_{1 \le j' \le l_b} \alpha^b_{j',0}) + (\sum\limits_{1 \le j' \le l_b} \beta^b_{j,j'} \alpha^b_{j',1}) d_1 + \dots + (\sum \limits_{1 \le j' \le l_b} \beta^b_{j,j'} \alpha^b_{j',n}) d_n.
\]
