...
\begin{itemize}
\item either for each cycle scheme $\schm$, all subexpressions in $\eval{O(q_m)}{\sumf^{(\schm,\sumf^{(H,\sval_0)})}}$ containing the cycle counter variables are identical to zero and hence can be ignored, 
\item or  there exists a cycle scheme $\schm$ such that $\eval{O(q_m)}{\sumf^{(\schm,\sumf^{(H,\sval_0)})}}$ is not identical to zero. 
\end{itemize}
...
We use $\sim_\initval$ to denote an equivalence relation over $[k]$, that is, over the set of indices for control variables. The intention of $\sim_\initval$ is as follows: $j'_1 \sim_\initval j'_2$ iff $\initval(x_{j'_1})=\initval(x_{j'_2})$, that is, the values of the control variables $x_{j'_1}$ and $x_{j'_2}$ assigned by $\initval$ are the same. 
...
%
\item For $i \ge 0$, $\Ee_{i+1}$ is the union of $\Ee_i$ and the set of equivalence relations $\sim'$, which is the reflexive, symmetric and transitive closure of $\left(\sim \cap\ (I^{\circled{C_j}}_{pe} \times I^{\circled{C_j}}_{pe})\right) \cup \{(j_1,j_2) \in I^{\circled{C_j}}_{tr} \times I^{\circled{C_j}}_{tr} \mid \pi^{\circled{C_j}}(j_1) = \pi^{\circled{C_j}}(j_2) \}$, for some $\sim \in \Ee_i$ and some cycle $C_j$ (where $j \in [n]$).
%
...
\framebox[\textwidth]{
	\begin{minipage}{0.95\textwidth}
		...
		\begin{enumerate}
		\item There is an equivalence class $J$ of $\sim$ such that $\sum \limits_{j' \in  J \cap I^{\circled{C_{i_1}}}_{pe}} \mu_{\schm,(i_1,j')} \neq 0$. 
		\item $\mu_{\schm,(i_1,0)} \neq 0$.
		\end{enumerate}
...
\smallskip
In the following, we present the arguments for the correctness of Step II.

If there exist an equivalence relation $\sim \in \Ee$ and an equivalence class $J$ of $\sim$ such that 
...
Then from $\schm=C_{i_1}^{\ell_1} C_{i_2} \dots C_{i_t}$, we can assign a sufficiently large value $s$ to $\ell_1$ so that the sum of the coefficients of the $\initval(x_{j'})$-atoms for $j' \in  J \cap I^{\circled{C_{i_1}}}_{pe}$ in $\eval{O(q_{m+1})}{\sumf^{(P\schm ,\sval_0})}$, which is equal to the sum of the term $\ell_1 \left(\sum \limits_{j' \in  J \cap I^{\circled{C_{i_1}}}_{pe}} \mu_{\schm,(i_1,j')}\right)$ and some constant,  becomes non-zero.  The guards in the path $P C_{i_1}^{s} C_{i_2} \dots C_{i_t}$ enforce a preorder over the subjects of those nontrivial non-constant atoms.
...
Otherwise, if $\eval{O(q_{m+1})}{\sumf^{(P\schm,\sval_0)}}$ contains some other nontrivial non-constant atoms, then we can apply a similar argument as above and conclude that $\eval{O(q_{m+1})}{\sumf^{(P\schm,\sval_0)}}$ can be made non-zero. On the other hand, if $\eval{O(q_{m+1})}{\sumf^{(P\schm,\sval_0)}}$ contains no nontrivial non-constant atoms, but $\mu_{\schm,(i_1,0)} \neq 0$, then we can let $\ell_1$ sufficiently large to make the expression $\eval{O(q_{m+1})}{\sumf^{(P\schm,\sval_0)}}$ non-zero. Therefore, when there exist an equivalence relation $\sim \in \Ee$ and an equivalence class $J$ of $\sim$ such that $\sum \limits_{j' \in  J \cap I^{\circled{C_{i_1}}}_{pe}} \mu_{\schm,(i_1,j')} \neq 0$, we are able to conclude that there \emph{must be} a path $P$ and an input to make $\eval{O(q_{m+1})}{\sumf^{(P\schm,\sval_0)}}$ non-zero.  
%Similar arguments can be applied to $\ell_2\dots\ell_n$.

\smallskip

If Step II does not return $\ltrue$, we show below that for all cycle schemes $\schm_1=C_{i_1}^{\ell_1} C_{i_2}^{\ell_2} \dots C_{i_{s_1}}^{\ell_{s_1}}$ with $i_1,i_2,\dots,i_{s_1} \in [n]$, all subexpressions containing cycle counter variables $\ell_1,\dots, \ell_{s_1}$ in $\eval{O(q_{m+1})}{\sumf^{(\schm_1, \sumf^{(H,\sval_0)}))}}$ are identical to zero and hence can be removed. Let ${i'_2} \dots {i'_{s_2}}$ be the sequence obtained from $i_2 \dots i_{s_1}$ by keeping just one copy for each duplicated index therein.  
In Step II we already checked the equivalence relation $\sim_H$ and the cycle scheme $\schm_2=C_{i_1}^{\ell_1} C_{i'_2} \dots C_{i'_{s_2}}$. Step II guarantees that all subexpressions containing $\ell_1$ in 
$\eval{O(q_{m+1})}{\sumf^{(\schm_2, \sumf^{(H,\sval_0)}))}}$ are identical to zero and hence can be removed.
Because for all $j\in[l]$, $\cstl^{^{\circled{C_1}}}_j, \dots, \cstl^{^{\circled{C_n}}}_j \in \{0,1\}$,   $(\lambda^{\circled{C_{i_2}}}_{j})^{\ell_2} \dots (\lambda^{\circled{C_{i_{s_1}}}}_{j})^{\ell_{s_1}} = \lambda^{\circled{C_{i'_2}}}_{j} \dots \lambda^{\circled{C_{i'_{s_2}}}}_{j}$. We proved that the $(\ast)$ and $(\ast\ast)$ style expressions are equivalent in both $\eval{O(q_{m+1})}{\sumf^{(\schm_1, \sumf^{(H,\sval_0)}))}}$ and $\eval{O(q_{m+1})}{\sumf^{(\schm_2, \sumf^{(H,\sval_0)}))}}$.
...
Those subexpressions containing $\ell_2$ can also be removed by considering the equivalence relation induced by the path $HC_{i_1}$ the cycle scheme $\schm_3=C_{i_2}^{\ell_2} C_{i''_3} \dots C_{i''_{s_3}}$ and applying a similar reasoning, where the sequence ${i''_3} \dots {i''_{s_3}}$ is obtained from ${i_3} \dots  i_{s_1}$, similarly to the construction of ${i'_2} \dots {i'_{s_2}}$ from $i_2 \dots i_{s1}$. The same applies to all other cycle counter variables $\ell_3,\dots,\ell_{s_1}$.
...
From Proposition~\ref{prop-sum-cycle},  we can observe that for each cycle $C_i$ (where $1 \le i \le n$),  the expression $\sumf^{(C^\ell_i, \initval)^-}(y_j)$ for $y_j \in Y$ is of one of the the following forms.
\begin{itemize}
\item If $\cstl^{\circled{C_i}}_{j}=0$, then $\sumf^{(C^\ell_i,\initval)^{-}}(y_j) = \sumf^{(C^\ell_i,\initval)}(y_j)$.
%\medskip\\
%\resizebox{0.9\hsize}{!}{$
...
\begin{array}{l }
\sumf^{(C^\ell_i,\initval)^-}(y_j)  =    \initval(y_j) +   
\sum \limits_{j' \in I^{\circled{C_i}}_{tr}} \csta^{\circled{C_i}}_{j,j'} \initval(x_{j'}) +  \sum \limits_{j' \in \rng(\pi^{\circled{C_i}})} \sum \limits_{s\in[\ell -1]}
...
Going one step further, we are able to show that the coefficients of the atoms in ${\sumf^{(\schm, \sumf^{(H,\initval_0)})}}^-(y_j)$ for all cycle schemes $\schm$ are from a bounded set.
%
...
\begin{itemize}
\item Suppose $(0, c_1, \dots, c_l) \in \Lambda$. Then $(0, c'_1, \dots, c'_l) \in \Lambda'$, where for each $j \in [l]$, if $\cstl^{\circled{C_{i'}}}_j = 0$, then $c'_j = \cste^{\circled{C_{i'}}}_{j}$, otherwise, $c'_j = c_j$.
%
\item For each $j' \in [k]$, suppose $(j', c_{j',1}, \dots, c_{j',l}) \in \Lambda$, do the following. 
\begin{itemize}
\item If $j' \in I^{\circled{C_{i'}}}_{pe}$, then $(j', c'_{j', 1}, \dots, c'_{j', l}) \in \Lambda'$,  where for each $j \in [l]$, if $\cstl^{\circled{C_{i'}}}_j = 0$, then $c'_{j', j}= \csta^{\circled{C_{i'}}}_{j, j'}$, otherwise, $c'_{j', j} = c_{j',j}$.
%
\item If $j' \in I^{\circled{C_{i'}}}_{tr}$, then {\small $(k+1, c'_{j',1}, \dots, c'_{j',l}), (j', \cstb^{\circled{C_{i'}}}_{1, \pi^{\circled{C_{i'}}}(j')}, \dots, \cstb^{\circled{C_{i'}}}_{l, \pi^{\circled{C_{i'}}}(j')}) \in \Lambda'$}, where for each $j \in [l]$, if $\cstl^{\circled{C_{i'}}}_j = 0$, then $c'_{j', j}= \csta^{\circled{C_{i'}}}_{j, j'}$, otherwise, $c'_{j', j} = c_{j',j} +  \csta^{\circled{C_{i'}}}_{j, j'}$.
%
\end{itemize}

\item For each tuple $(k+1, c_1, \dots, c_l) \in \Lambda$, $(k+1, c_1, \dots, c_l) \in \Lambda'$. In addition, for each $j' \in [r^{\circled{C_{i'}}}] \setminus \rng(\pi^{\circled{C_{i'}}})$, $(k+1, \cstb^{\circled{C_{i'}}}_{1, j'}, \dots, \cstb^{\circled{C_{i'}}}_{l, j'}) \in \Lambda'$. 
\end{itemize}
\end{itemize}
\end{enumerate}
%
%We first compute $\abs(HC_1), \ldots \abs(HC_n)$, put them into $\mathscr{A}$, then compute from them the abstractions $\abs(HC_1C_1),\dots, \abs(HC_1C_n), \abs(HC_2C_1), \dots$ by appending $C_1,\dots,C_n$, put them into $\mathscr{A}$, and so on, until reaching a fixed point.
We are ready to present Step III.
\bigskip\\
\framebox[\textwidth]{
	\begin{minipage}{0.95\textwidth}
...
\noindent {\it Complexity analysis of Step III}. The size of the set $U$ is polynomial over the size of the generalized lasso (i.e. the size of the transitions in the generalized lasso). The size of $\Xi$ in $\mathscr{A}'$ is at most exponential over $k$ and $l$, and the size of $\Delta$ in $\mathscr{A}'$ is exponential over $l$. Therefore, the computation of $\mathscr{A}'$ takes time exponential over $k$ and $l$. The three conditions in Step III can be checked in time polynomial over the size of $\mathscr{A}'$. In summary, the complexity of Step III is exponential over the number of control and data variables.
