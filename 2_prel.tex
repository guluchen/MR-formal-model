...
For a function $\pi$, let $\dom(\pi)$ and $\rng(\pi)$ denote the \emph{domain}  and \emph{range} of $\pi$, respectively. We use $\emptyset$ to denote a function with empty domain. An \emph{expression} $e$ over the set of variables $Z$ is defined by the following rules, $e\equiv  c \mid  c z \mid (e + e) \mid (e - e)$, where $z \in Z$ and $c\in \intnum$.  As a result of the commutativity of $+$, without loss of generality, we assume that all expressions $e$ in this paper are of the form $c_0 + c_1 z_1 + \dots + c_n z_n$, where $c_0, c_1,\dots,c_n \in \intnum$ and $z_1,\dots,z_n \in Z$. For an expression $e=c_0+c_1 z_1 + \dots + c_n z_n$, let $\vars(e)$ denote the set of variables $z_i$ such that $c_i \neq 0$. Let $\Ee_Z$ denote the set of all expressions over the set of variables $Z$. 
