...
We prove the Theorem~\ref{thm:correctness} by presenting a decision procedure for the non-zero output problem of normalized SNTs. We fix a normalized SNT $\Ss = (Q,X,Y,\delta,q_0,O)$ such that $X=\{ x_1,\dots, x_k\}$ and $Y = \{y_1,\dots,y_l\}$. Due to space constraint, we only present a simplified version where the transition guards are constant-free and leave the procedure for the general class in the appendix.
...
\hide{Before presenting the decision procedure for generalized lassos, we introduce some notations.
...
Let $e$ be an expression consists of symbolic values $\initval(z)$ for $z\in X\cup Y$ and data variables $\vard_1, \dots, \vard_m$. More specifically, let $e:=\mu_0 + \mu_1 \initval(z_1) +\dots + \mu_n \initval(z_n) + \xi_1 \vard_1 + \dots + \xi_m \vard_m$,
such that $\mu_0,\mu_1,\dots,\mu_n, \xi_1,\dots,\xi_m$ are the expressions that contain only constants and loop counter variables (denoted by variants of the symbol $\ell$).
Then we call $\mu_0$ as the \emph{constant atom}, $\mu_i \initval(z_i)$ the $\initval(z_j)$-atom for $i\in[n]$, and $\xi_j \vard_j$ the $\vard_j$-atom for $j\in[m]$ of the expression $e$. Moreover, $\mu_1, \dots, \mu_n, \xi_1,\dots, \xi_m$ are called the \emph{coefficients} of these atoms. A non-constant atom is said to be \emph{nontrivial} if its coefficient is \emph{not} identical to zero.
...
\end{lemma}
Notice that in the above, $\lambda^{\circled{C_{i_n}}}_j\in\{0,1\}$ for $j\in[l]$ and $n\in [t]$. Hence the value of $(1+\lambda^{\circled{C_{i_n}}}_{j} + \dots + (\lambda^{\circled{C_{i_n}}}_{j})^{\ell_n-1} )$ can only be $1$ or $\ell_n$ and $\left(\lambda^{\circled{C_{i_m}}}_{j}\right)^{\ell_m}\in\{0,1\}$.
Hence both the constant atom and the coefficient of the $\initval(x_{j'})$-atom (for $j'\in [l]$) can be rewritten to the form of $c_0+c_1l_1+c_2l_2+\ldots+c_nl_t$ for $c_0\ldots c_n\in \intnum$. 




We are ready to present the decision procedure. By the ``well-defined'' and ``unique-valued'' constraints of a normalized SNT, without loss of generality, we assume that $I^{\circled{H}}_{tr}=[k]$, that is, after traversing $H$, the values of all control variables become defined.
Under the assumption, for all $j \in [k]$, the symbolic value of $\sumf^{(H,\emptyset)}(x_j)=\vard^{\circled{H}}_{\pi^{\circled{H}}(j)}$\smallskip\\
\framebox[\textwidth]{
\begin{minipage}{0.95\textwidth}
\noindent {\bf Step I}. Decide whether $\eval{O(q_m)}{\sumf^{(H,\emptyset)}}$ is not identical to zero.
This can be done by checking if the constant-atom or the coefficient of some non-constant atom of $\eval{O(q_m)}{\sumf^{(H,\emptyset)}}$ is not identical to zero.
If the answer is yes, then the decision procedure terminates and returns the answer $\ltrue$. Otherwise, go to Step II.
\end{minipage}
}\bigskip
%}

%\in [k]$ (recall that we assume $I^{\circled{H}}_{tr}=[k]$), $\sumf^{(H,\initval_0)}(x_{j'})=\vard^{\circled{H}}_{\pi^{\circled{H}}(j')}$. 

The goal of Step II is either showing that in the analysis of $f=\eval{O(q_m)}{\sumf^{(\schm,\sumf^{(H,\emptyset)})}}$ all subexpressions containing cycle counter variables are identical to zero and hence can be ignored or showing that $f$ is not identical to zero. Let $\schm=C_{i_1}^{\ell_1} C_{i_2}^{\ell_2} \dots C_{i_t}^{\ell_t}$ be a cycle scheme. Then from Lemma~\ref{prop-cycle-schm}, for each $j' \in  I^{\circled{C_{i_1}}}_{pe}$, the coefficient of the $\initval(x_{j'})$-atom of $\eval{O(q_m)}{\sumf^{(\schm,\initval)}}$ contains the following subexpression, 
\begin{center}
	\resizebox{0.7\hsize}{!}{$
...
Similarly, from Lemma~\ref{prop-cycle-schm},  the constant atom of  $\eval{O(q_m)}{\sumf^{(\schm,\initval_0)}}$ contains the following  subexpression,
\begin{center}
	\vspace{-0.2cm}
	\resizebox{0.7\hsize}{!}{$
...
\framebox[\textwidth]{
	\begin{minipage}{0.95\textwidth}
		\noindent {\bf Step II}. For each $i_1: 1 \le i_1 \le n$, check all cycle scheme $\schm=C_{i_1}^{\ell_1} C_{i_2} \dots C_{i_t}$ such that $i_2,\dots,i_t$ are mutually distinct. There are only finitely many this kind of cycle schemes. If 
...
%If $j' \in  I^{\circled{C_{i'_2}}}_{pe}$, then the we have subexpressions containing $\ell_2$ in the coefficient of the $\initval(x_{j'})$-atom (when $j' \in  I^{\circled{C_{i'_1}}}_{pe}$) or $\vard^{\circled{C_{i'_1}}}_{\pi^{\circled{C_{i'_1}}}(j')}$-atom (when $j' \in  I^{\circled{C_{i'_1}}}_{tr}$) of $\eval{O(q_m)}{\sumf^{(\schm,\initval)}}$.
%Also we may have subexpressions containing $\ell_2$ in the constant atom.
...
For a cycle scheme $\schm$ and $y_j \in Y$, we use the notation ${\sumf^{(\schm,\initval)}}^-(y_j)$ to denote the expression obtained by removing from the coefficients of the atoms of $\sumf^{(\schm,\initval)}(y_j)$ all subexpressions containing cycle counting variables. 

\begin{lemma}\label{prop-bnd-domain-1}
	Suppose that the decision procedure has not returned $\ltrue$ after Step~II. For each cycle scheme $\schm$, let $f=\eval{O(q_m)}{\sumf^{(\schm, \sumf^{(H,\emptyset)})}}$ and $f'=\eval{O(q_m)}{{\sumf^{(\schm, \sumf^{(H,\emptyset)})}}^-}$. For all valuation $\rho$, $\eval{f}{\rho}\neq 0$ iff $\eval{f'}{\rho} \neq 0$.
\end{lemma}
...
We first define a \emph{generalized multi-lasso} as a sequence $\gmlasso= H_1 (C_{1,1},\dots,C_{1,n_1}) H_2 (C_{2,1},\dots,C_{2,n_2}) \dots H_r (C_{r,1},\dots, C_{r, n_r})$ s.t. (1) for each $s\in[r]$, $H_s = q_{s,1} \dots q_{s, m_s}$ and $H_s (C_{s,1},\dots,C_{s, n_s})$ is a generalized lasso, (2) \yfc{for each $s, s'\in[r]$ with $s < s'$, $H_s (C_{s,1},\dots,C_{s, n_s})$ and $H_{s'} (C_{s', 1},\dots,C_{s', n_{s'}})$ are state-disjoint}, (3) $q_{1,1}=q_0$, and (4) for each $s \in [r-1]$, $q_{s, m_s}=q_{s+1,1}$.
...
For each $s\in [r]$, suppose $H_s=q_{s,1} \dots q_{s, m_s}$. Without loss of generality, we assume that $O(q_{r,m_r})=a_0+a_1 x_1 + \dots + a_k x_k + b_1 y_1  + \dots + b_l y_l$ and $O(q')$ is undefined for every other state $q'$ in $\gmlasso$.\smallskip\\
...
\framebox[\textwidth]{
	\begin{minipage}{0.95\textwidth}
		\noindent {\bf Step I$'$}. We do the same analysis of Step I for the path $H_1\dots H_r$.
	\end{minipage}
}\smallskip

Let $s\in [2,r]$. In order to analyze the set of cycles $(C_{s-1,1},\dots,C_{s-1,n_1})$, below we show how to summarize effect of the path $H_s\dots H_r$ to the output state $q_{r,m_r}$ to the the state $q_{s-1, m_{s-1}}$, which is shared by all those cycles.
Suppose in $\eval{O(q_{r,m_r})}{\sumf^{(H_s\dots H_{r}, \initval)}}$, the constant atom is $a_{s,0}$, the coefficient of the $\initval(x_j)$-atom is $a_{s, j}$ for each $j \in [k]$, and the coefficient of the $\initval(y_{j'})$-atom is $b_{s, j'}$ for each $j' \in [l]$. Then we change the output function and let
$O(q_{s-1, m_{s-1}}):=a_{s, 0}+a_{s, 1} x_1 + \dots + a_{s, k} x_k + b_{s,1} y_1 + \dots + b_{s, l} y_l.$\smallskip\\
\framebox[\textwidth]{
	\begin{minipage}{0.95\textwidth}
\noindent {\bf Step II$'$}.  For each $s\in [r]$ and $m\in [n_s]$, we check each cycle scheme $\schm = C^{\ell_1}_{s,m} C_{i_{2}} \dots C_{i_{t}}$ such that $C_{i_{2}} \dots C_{i_{t}}\in \{C_{s, 1}, \dots, C_{s,n_s},\dots, C_{r,1}, \dots, C_{r,n_r}\}$ and $C_{i_{2}} \dots C_{i_{t}}$ are mutually distinct by performing an analysis of the expression $\eval{ O(q_{s, m_{s}})} {\sumf^{(\schm,\sumf^{(H_1 \dots H_{s}, \initval_0)} ) } }$, in a way similar to Step II. If the decision procedure does not return during the analysis, then go to Step III$'$.
	\end{minipage}
}\smallskip

Intuitively, in Step II$'$, during the analysis of $\eval{ O(q_{s, m_{s}})} {\sumf^{(\schm,\sumf^{(H_1 \dots H_{s}, \initval_0)} ) } }$, the effect of the paths $H_{s+1},  \dots,  H_r$ and the cycles $C_{i_{2}}, \dots, C_{i_{t}}$ is described by the expressions $\cstl^{\circled{H_{s+1}}}_j \dots \cstl^{\circled{H_{r}}}_j  \cstl^{\circled{C_{i_{s,2}}}}_j \dots \cstl^{\circled{C_{i_{s,t}}}}_j $ for $j \in [l]$. Since $O(q_{s, m_{s}})$ has already taken into consideration the expressions $\cstl^{\circled{H_{s+1}}}_j \dots \cstl^{\circled{H_{r}}}_j$ for $j \in [l]$, conceptually, in Step II$'$, we can do the analysis as if we have a generalized lasso where the handle is $H_1\dots H_s$ and the collection of cycles is $\{C_{s,1},\dots, C_{s,n_s}$, $\dots$, $C_{r,1},\dots, C_{r,n_r}\}$. 
...
After Step II$'$, if the decision procedure has not returned yet, then similar to Proposition~\ref{prop-bnd-domain-2}, the following results hold.
\begin{itemize}
\item For each $s \in [r]$ and each path $\schm=H_1 \schm_1 H_2 \dots H_s \schm_s$ such that for each $s'\in [s]$, $\schm_{s'}$ is a cycle scheme over the collection of cycles $\{C_{s',1},\dots,C_{s',n_{s'}}\}$, it holds that the constant atom and all the coefficients of the non-constant atoms in ${\sumf^{(\schm,\initval_0)}}^-(y_j)$ are from a bounded domain $U$.
%
