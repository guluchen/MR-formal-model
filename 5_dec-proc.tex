...
In the above, $j'\in  I^{\circled{P_1}}_{pe}$ implies that $x_{j'}$ remains unchanged when traversing $P_1$, which means the initial value of $x_{j'}$ before traversing $P_2$ is still $\initval(x_{j'})$ and therefore we have the item $ \left(\csta^{\circled{P_2}}_{j,j'}\right) \initval(x_{j'})$. When $j' \in \rng(\pi^{\circled{P_1}})$, the initial value of $x_{(\pi^{\circled{P_1}})^{-1}(j')}$ before traversing $P_2$ is $\vard^{\circled{P_1}}_{j'}$ and therefore we have the item $\left( \csta^{\circled{P_2}}_{j,(\pi^{\circled{P_1}})^{-1}(j')}\right) \vard^{\circled{P_1}}_{j'}$.
...
such that 
\begin{itemize}
\item for each $i: 1 \le i \le r$, $H_i=q_{i,1} \dots q_{i, m_i}$ and $H_i (C_{i,1},\dots,C_{i,n_i})$ is a generalized lasso, 

\item for each $i,j: 1 \le i < j \le r$, $H_i (C_{i,1},\dots,C_{i,n_i})$ and $H_j (C_{j,1},\dots,C_{j, n_j})$ are state-disjoint,

\item $q_{1,1}=q_0$, and for each $i: 1 \le i < r$, $q_{i, m_i}=q_{i+1,1}$.
\end{itemize}
...
\gmlasso= H_1 (C_{1,1},\dots,C_{1,n_1}) H_2 (C_{2,1},\dots,C_{2,n_2}) \dots H_r (C_{r,1},\dots, C_{r, n_r}).
\]
For each $i: 1 \le i \le r$, suppose $H_i=q_{i,1} \dots q_{i, m_i}$. Without loss of generality, we assume that $O(q_{r,m_r})=a_0+a_1 x_1 + \dots + a_k x_k + b_1 y_1  + \dots + b_l y_l$ and $O(q')$ is undefined for every other state $q'$ in $\gmlasso$.
...
For each $i: 2 \le i \le r$, suppose  that 
in $\sub{O(q_{r,m_r})}{\sumf^{(H_i\dots H_{r}, \initval)}}$, the constant atom is $a'_{i,0}$, the coefficient of the $\initval(x_j)$-atom is $a'_{i,j}$ for each $j \in [k]$, moreover, the coefficient of the $\initval(y_{j'})$-atom is $b'_{i,j'}$ for each $j' \in [l]$. Then we change the output function by setting 
\[O(q_{i-1, m_{i-1}})=a'_{i,0}+a'_{i,1} x_1 + \dots + a'_{i,k} x_k + b'_{i,1} y_1 + \dots + b'_{i,l} y_l.\]
