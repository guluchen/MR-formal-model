...
Before presenting the decision procedure for generalized lassos, we introduce an additional notation.

Suppose $P$ is a path in $\Ss$, $\Omega$ is a symbolic valuation representing the initial values of the control and data variables, and $r^{(P)}$ data values are introduced when traversing $P$. Moreover, let $e$ be an expression that is linear over $\Omega(x_1),\dots,\Omega(x_k), \vard^{(P)}_1,\dots,\vard^{(P)}_{r^{(P)}}$, more precisely, $e:=\mu_0 + \mu_1 \Omega(x_1) +\dots + \mu_k \Omega(x_k) + \nu_1 \vard^{(P)}_{1} + \dots + \nu_{r^{(P)}} \vard^{(P)}_{r^{(P)}}$, where $\mu_0,\mu_1,\dots,\mu_k,\nu_1,\dots,\nu_{r^{(P)}}$ are the expressions that contain none of the variables from $\Omega(x_1), \dots, \Omega(x_k), \vard^{(P)}_{1},\dots, \vard^{(P)}_{r^{(P)}}$. Then we call the subexpressions $\mu_0,\mu_1 \Omega(x_1),\dots,\mu_k \Omega(x_k),\nu_1 \vard^{(P)}_{1} ,\dots, \nu_{r^{(P)}} \vard^{(P)}_{r^{(P)}}$ as \emph{atoms of $e$ with respect to $P$}. 

Suppose that $\Ss$ comprises a handle $H=q_0 q_1 \dots q_m$ and a collection of simple cycles $C_1,\dots,C_n$ which share the unique state $q_m$. Moreover, without loss of generality, we assume that $O(q_m) = a_0 + a_1 x_1 + \dots + a_k x_k + b_1 y_1 + \dots + b_l y_l$, and $O(q)$ is undefined for all the other states $q$.
