...
The coefficients of the loop counter variable $\ell$ can be none-zero when $\cstl^{\circled{C}}_{j}=1$. The coefficients may propagate to the output expression.  In such a case, due to the ``transition-enablement'' constraint of a normalized SNT (for any sequence of transitions, one can find a corresponding run), one can pick a run corresponding to a very large $\ell$ such that it dominates the value of the output expression. Therefore the the output of the SNT can be made none-zero. In the decision procedure we are going to present, we first check if the handle of the generalized lasso produces none-zero output in Step I.
We then check if the coefficient of $\ell$ is none-zero in the output expression in Step II. If the coefficient of $\ell$ is zero in the output expression, then we show in Step III that the testing of output none-zeroness can be reduced to a finite state reachability problem and thus can be easily decided.
