...
\begin{proposition}\label{prop-cycle-schm}
Suppose $\schm=C_{i_1}^{\ell_1} C_{i_2}^{\ell_2} \dots C_{i_t}^{\ell_t}$ is a cycle scheme, and $\initval$ is a symbolic valuation representing the initial values of the control and data variables. Let $r_j$ be the largest number that $j\in\bigcap_{n\in[r_j]} I^{\circled{C_{i_{n}}}}_{pe}$, i.e., $x_j$ remains persistent after traversing $C_{i_1}^{\ell_1} C_{i_2}^{\ell_2} \dots C_{i_{b_j}}^{\ell_{b_j}}$.
...
Moreover, for each $j: 1 \le j \le l$, it holds that only the constant atom and the coefficient of the $\initval(x_{j'})$-atom (for $j': 1 \le j' \le l$) in $\sumf^{(\schm,\initval)}(y_j)$ contain a subexpression of the form $c \ell_1$.
\end{proposition}
...
We are ready to present the decision procedure. By the ``well-defined'' and ``unique-valued'' constraint of a normalized SNT, without loss of generality, we assume that $I^{\circled{H}}_{tr}=[k]$, that is, after traversing $H$, the values of all control variables become defined.
...
This can be done by checking if the constant-atom or the coefficient of some non-constant atoms is not identical to zero.
...
\]
%
The expression $(\ast\ast)$ can be rewritten as $\mu_{\schm,(i_1,0)} \ell_1 + \nu_{\schm,(i_1,0)}$ for some integer constants $\mu_{\schm, (i_1,0)}$ and $\nu_{\schm, (i_1,0)}$. If both $\mu_{\schm,(i_1,0)}=\mu_{\schm,(i_1,j')}=0$, then we can ignore subexpression containing the cycle counter variable $\ell_1$ (i,e, the expressions $(\ast)$ and $(\ast\ast)$)  $\eval{O(q_m)}{\sumf^{(\schm,\initval)}}$\smallskip\\
\framebox[\textwidth]{
	\begin{minipage}{0.95\textwidth}
		\noindent {\bf Step II}. For each $i_1: 1 \le i_1 \le n$, check all cycle scheme $\schm=C_{i_1}^{\ell_1} C_{i_2} \dots C_{i_t}$ such that $i_2,\dots,i_t$ are mutually distinct and 
...
If Step II does not return $\ltrue$, we show that all cycle counter variables can be ignored in the analysis of any cycle scheme $\schm=C_{i'_1}^{\ell_1} C_{i'_2}^{\ell_2} \dots C_{i'_n}^{\ell_n}$.
In Step II we already checked a cycle scheme $\schm'=C_{i'_1}^{\ell_1} C_{i''_2} \dots C_{i''_m}$, where 
$C_{i''_2}, \dots, C_{i''_m}$ contain all cycles in $C_{i'_2} ,\dots, C_{i'_n}$. Step II guarantees that we can ignore the subexpressions containing $\ell_1$ in the form of $(\ast)$ and $(\ast\ast)$ in 
$\eval{O(q_m)}{\sumf^{(\schm',\initval)}}$.
We show that we can do the same for $\eval{O(q_m)}{\sumf^{(\schm,\initval)}}$.
Notice that $j: 1 \le j \le l$, $\cstl^{^{\circled{C_1}}}_j, \dots, \cstl^{^{\circled{C_n}}}_j \in \{0,1\}$, therefore we have $(\lambda^{\circled{C_{i'_2}}}_{j})^{\ell_2} \dots (\lambda^{\circled{C_{i'_n}}}_{j})^{\ell_n} = \lambda^{\circled{C_{i''_2}}}_{j} \dots \lambda^{\circled{C_{i''_m}}}_{j}$. Hence the $(\ast)$ and $(\ast\ast)$ style expressions are equivalent in both $\schm$ and $\schm'$ and thus can both be ignored.
If $j' \in  I^{\circled{C_{i'_2}}}_{pe}$, then the we have subexpressions containing $\ell_2$ in the coefficient of the $\initval(x_{j'})$-atom of $\eval{O(q_m)}{\sumf^{(\schm,\initval)}}$.
Also we may have subexpressions containing $\ell_2$ in the constant atom.
Those subexpressions containing $\ell_2$ can also be ignore by considering the cycle scheme $\schm'=C_{i'_2}^{\ell_3} C_{i''_3} \dots C_{i''_m}$ and applying a similar reasoning. The same applied to all other cycle counting variables $\ell_3,\dots,\ell_n$.

\hide{We would like to remark that in Step II, for a given index $i_1$, we restrict the cycle schemes to those of the form $C_{i_1}^{\ell_1} C_{i_2} \dots C_{i_t}$. We show that this is enough to cover all cycle schemes beginning with $C_{i_1}^{\ell_1}$ by the following argument: For each $j: 1 \le j \le l$, $\cstl^{^{\circled{C_1}}}_j, \dots, \cstl^{^{\circled{C_n}}}_j \in \{0,1\}$. Therefore, for each cycle scheme $\schm=C_{i_1}^{\ell_1} C_{i_2}^{\ell_2} \dots C_{i_t}^{\ell_t}$ and each $j: 1 \le j \le l$,  we have $(\lambda^{\circled{C_{i_2}}}_{j})^{\ell_2} \dots (\lambda^{\circled{C_{i_t}}}_{j})^{\ell_t} = \lambda^{\circled{C_{i_2}}}_{j} \dots \lambda^{\circled{C_{i_t}}}_{j}$.}
