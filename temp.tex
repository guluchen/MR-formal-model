...
We illustrate the arguments for the case $\cstl^{(C_{i_t})}_{j} = 1$. The case $\cstl^{(C_{i_t})}_{j} = 0$ is simpler and can be discussed similarly. Suppose $y_j \in Y$.  In the following, we check that the constant atom and the coefficients of all the non-constant atoms of ${\sumf^{(\schm,\sumf^{(H,\initval_\bot)})}}^-(y_j)$ belong to $K$.  
%For brevity, we abbreviate an(resp. the) atom of ${\sumf^{(\schm,\sumf^{(H,\initval_\bot)})}}^-(y_j)$ as an (resp. the) atom below.
\begin{itemize}
	\item $(\cste^{\circled{\schm}}_{j})' = 0 + (\cstl^{(C_{i_t})}_{j})^{\ell_t} (\cste^{(\schm_1)}_j)' = (\cste^{(\schm_1)}_j)' \in K$ (here $\cste^{(C_{i_t})}_{j} \ell_t$ is removed).
	%
	%\item the coefficient of $o_j$ is $1$,
	%
	\item For each $j' \in [r^{\circled{H}}]$ such that there does not exist $j'' \in [k]$ satisfying that $(\sumf^{(\schm_1,\sumf^{(H,\initval_0)})})'(x_{j''})=\vard^{\circled{H}}_{j'}$, $(\csta^{\circled{\schm}}_{j, j'})' = (\cstl^{(C_{i_t})}_{j})^{\ell_t} (\csta^{(\schm_1)}_{j, j'})' = (\csta^{(\schm_1)}_{j, j'})' \in K$.
	%
	\item For each $j' \in [r^{\circled{H}}]$ such that $(\sumf^{(\schm_1,\sumf^{(H,\initval_0)})})'(x_{j''})=\vard^{\circled{H}}_{j'}$ for some $j''  \in I^{(C_{i_t})}_{pe}$, $(\csta^{\circled{\schm}}_{j, j'})' = 0 + (\cstl^{(C_{i_t})}_{j})^{\ell_t} (\csta^{(\schm_1)}_{j, j'})'= (\csta^{(\schm_1)}_{j, j'})' \in K$ (here $\csta^{(C_{i_t})}_{j,j'} \ell_t$ is removed). In this case, we have ${\sumf^{(\schm,\sumf^{(H,\initval_\bot)})}}^-(x_{j''})=\vard^{\circled{H}}_{j'}$. From the induction hypothesis, it is easy to see that the second result holds for $\schm$, $y_j$.
	%
	\item For each $j' \in [r^{\circled{H}}]$ such that $(\sumf^{(\schm_1,\sumf^{(H,\initval_0)})})'(x_{j''})=\vard^{\circled{H}}_{j'}$ for some $j''  \in I^{(C_{i_t})}_{tr}$, $(\csta^{\circled{\schm}}_{j, j'})' =(\cstl^{(C_{i_t})}_{j})^{\ell_t-1} \csta^{(C_{i_t})}_{j,j''} + (\cstl^{(C_{i_t})}_{j})^{\ell_t} (\csta^{(\schm_1)}_{j, j'})' = \csta^{(C_{i_t})}_{j,j''} + (\csta^{(\schm_1)}_{j, j'})' $. From the induction hypothesis, we know that either $(\csta^{(\schm_1)}_{j, j'})' = \csta^{(C_{i_{s_0}})}_{j,j''}$ if there is $s$ such that $\cstl^{(C_{i_s})}_j =0$,  or otherwise, $(\csta^{(\schm_1)}_{j, j'})'=\cstb^{\circled{H}}_{j, j'}$. Therefore, $(\csta^{\circled{\schm}}_{j, j'})' =\csta^{(C_{i_t})}_{j,j''} +  \csta^{(C_{i_{s_0}})}_{j,j''}$ or $\csta^{(C_{i_t})}_{j,j''}+ \cstb^{\circled{H}}_{j, j'}$. We conclude that $(\csta^{\circled{\schm}}_{j, j'})' \in K$.
	%
	...
	%
	\item For each $s \in [t-1]$, $i \in [\ell_s]$, and $j' \in [r^{(C_{i_s})}]$ such that $(\sumf^{(\schm_1,\sumf^{(H,\initval_0)})})'(x_{j''}) = \vard^{(C_{i_s}, i)}_{s, j'}$ for some $j'' \in I^{(C_{i_t})}_{tr}$, it holds that $i = \ell_s$, $j'' \in I^{(C_{i_s})}_{tr}$, and for each $s': s < s' \le t$, $j'' \in I^{(C_{i_{s'}})}_{pe}$. Then $(\cstb^{(\schm,i)}_{s,j'})' =(\cstl^{(C_{i_t})}_{j})^{\ell_t}  (\cstb^{(\schm_1, i)}_{s, j'})' + \csta^{(C_{i_t})}_{j, j''}  =  (\cstb^{(\schm_1, i)}_{s, j'})' + \csta^{(C_{i_t})}_{j, j''}$. From the induction hypothesis,  if there is $s': s < s' \le t-1$ such that $\cstl^{(C_{i_{s'}})}_j =0$, let $s'_0$ be the maximum $s'$ satisfying the constraint, then $(\cstb^{(\schm_1, i)}_{j,j'})'=\csta^{(C_{i_{s'_0}})}_{j, j''}$, otherwise, $(\cstb^{(\schm_1, i)}_{j,j'})'= \cstb^{(C_{i_s})}_{j,j'}$. Therefore, $(\cstb^{(\schm,i)}_{s,j'})' = \csta^{(C_{i_{s'_0}})}_{j, j''} + \csta^{(C_{i_t})}_{j, j''}$ or $\cstb^{(C_{i_s})}_{j,j'} + \csta^{(C_{i_t})}_{j, j''}$, thus belongs to $K$. In this case, ${\sumf^{(\schm,\sumf^{(H,\initval_\bot)})}}^-(x_{j''})=\vard^{(C_{i_t},\ell_t)}_{t, \pi^{(C_{i_t})}(j'')}$ and $(\cstb^{\circled{\schm,\ell_t}}_{t, \pi^{(C_{i_t})}(j'')})' = \cstb^{(C_{i_t})}_{j, \pi^{(C_{i_t})}(j'')}$. So the third condition holds for $\schm$ and $y_j$.
	%
...
\hide{
	...
	Let $\schm=C^{\ell_1}_{i_1} \dots C^{\ell_t}_{i_t}$ be a cycle scheme. An abstraction of ${\sumf^{(\schm,\sumf^{(H,\initval_\bot)})}}^-$, denoted by $\abs\circled{\schm}$, can be computed from $\schm$ by the following procedure.
	\begin{enumerate}
		\item Let $\abs\circled{\schm}:=\{(0, ( (\cste^{\circled{\schm}}_{1})',\dots, (\cste^{\circled{\schm}}_l)'))\}$.
		%
		\item For each $j \in [k]$ and $j' \in [l]$, let $\cstg_{j, j'}$ be the coefficient of the $({\sumf^{(\schm,\sumf^{(H,\initval_\bot)})}}^-(x_j))$-atom in ${\sumf^{(\schm,\sumf^{(H,\initval_\bot)})}}^-(y_{j'})$. Let $\abs\circled{\schm}: = \abs\circled{\schm} \cup \{(j, (\cstg_{j,1},\dots, \cstg_{j,l})) \mid j \in [k]\}$.
		%
		\item For each data variable $\vard'$ such that $\vard' \neq {\sumf^{(\schm,\sumf^{(H,\initval_\bot)})}}^-(x_j)$ for each $j \in [k]$, let $\abs\circled{\schm}:=\abs\circled{\schm} \cup \{(k+1, (c_1,\dots,c_l))\}$, where $(c_1,\dots,c_l) \in K^l$ is the tuple of the coefficients of the $\vard'$-atom in $({\sumf^{(\schm,\sumf^{(H,\initval_\bot)})}}^-(y_{j'})$ for $j' \in [l]$. 
	\end{enumerate}
	Intuitively, $\abs\circled{\schm}$, the abstraction of  ${\sumf^{(\schm,\sumf^{(H,\initval_\bot)})}}^-$,  is the set comprising the tuple of the constant atoms, the tuples of the coefficients of the atoms corresponding to the control variables, and the tuples of the coefficients of the atoms corresponding to the other data variables.
	
	From the proof that all the constant atoms and the coefficients of the non-constant atoms in ${\sumf^{(\schm,\sumf^{(H,\initval_\bot)})}}^-(y_j)$ for $y_j \in Y$ are from a bounded domain $K$, it is not hard to see that $\Kk = \{\abs\circled{\schm} \mid \schm \mbox{ a cycle scheme}\}$ can be computed effectively from $\sumf^{(H,\initval)}$, $\sumf^{(C_1,\initval)}$, $\dots$, and $\sumf^{(C_n,\initval)}$.\qed}
\hide{
	At first,  let us check $\chi^{\circled{C_{i_1}}}_{\ell_1}(O(q_m))$.
	\[
	\begin{array}{l c l}
	\chi^{\circled{C_{i_1}}}_{\ell_1}(O(q_m)) & = & a_0 + a_1 \chi^{\circled{C_{i_1}}}_{\ell_1}(x_1) + \dots a_k \chi^{\circled{C_{i_1}}}_{\ell_1}(x_k) + \\
	& & b_1 \chi^{\circled{C_{i_1}}}_{\ell_1}(y_1) + \dots + b_l \chi^{\circled{C_{i_1}}}_{\ell_1}(y_l).
	\end{array}
	\] 
	
	Then $\chi^{\circled{C_{i_1}}}_{\ell_1}(O(q_m))$ is a linear combination of the variables $d^{(0)}_1,\dots, d^{(0)}_{r_H}$ and $d^{\circled{C_{i_1},1}}_1,\dots, d^{\circled{C_{i_1},1}}_{r_{C_{i_1}}}, \dots, d^{(C_{i_1},\ell_1)}_1,\dots, d^{(C_{i_1},\ell_1)}_{r_{C_{i_1}}}$.
...
\hide{
	...
	Let $U$ be the subset of $\intnum$ comprising the following numbers,
	\begin{itemize}
		...
		%$j' \in I^{\circled{C_{i_1}}}_{tr}$, and $\cstl^{(C_{i_2})}_j = 0$.
	\end{itemize}}
