...
%Similarly, $(\sumf^{(\schm_1,\sumf^{(H,\initval_0)})})'(y_j)$ is a linear combination of $\vard^{(H)}_1$, $\dots$, $\vard^{(H)}_{r^{(H)}}$, $\vard^{(C_{i_1},1)}_{1,1}$, $\dots$, $\vard^{(C_{i_1},\ell_1)}_{1,r^{\circled{C_{i_1}}}}$, $\dots$, $\vard^{(C_{i_{t-1}},1)}_{t-1,1}$, $\dots$, $\vard^{(C_{i_{t-1}},\ell_{t-1})}_{t-1, r^{(C_{i_{t-1}})}}$.
