...
%
\item Suppose $x_1$ (resp. $x_2$) computes neither  the minimum value nor the maximum value in $S$. Then the value of $x_1$ and $x_2$ are unchanged when staying in $S$. Therefore, if $x_1 \le x_2$ holds in the state $(q, \preceq_1)$, then it holds in each state belonging to the same SCC as $(q, \preceq_1)$ in $\Ss'$. In particular, $(x_1, x_2) \in \preceq_2$, a contradiction.
\end{itemize}
