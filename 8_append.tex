...
The fact that the SNT $\Ss_p$ is generalized flat follows from the requirement that in each statement of the form $\mbox{if}\ (g)\ \{s'_1\}$, $\mbox{if}\ (g)\ \{s'_1\}\ \mbox{else}\ \{s'_2\}$, it holds that the statement $\nnext;$ does not occur in $s'_1$ or $s'_2$. This requirement guarantees that in a program $p$ of the form $s_1; \nnext; \loopL{s_2;\nnext;}$; ret $r$, it holds that every execution path in $s_1;\nnext;$ (resp. $s_2;\nnext;$) contain the same number of $\nnext;$ statements, which implies that all the maximal paths in the SCC-graph of the transition graph of $\Ss_p$ have the same length.
\qed
\end{proof}
...
\end{itemize}
\end{itemize}
From the aforementioned construction, it is easy to see that $\Ss_1$ and $\Ss_2$ are  inequivalent iff there is a data word $w$ such that the output of $\Ss$ over $w$ is non-zero.

In the following, we show that each monotone SNT $\Ss=(Q, X, Y, \delta, q_0, O)$ can be normalized into a monotone SNT $\Ss'=(Q', X, Y, \delta', q'_0, O')$ so that $\Ss'$ is transition-enabled and at the same time still generalized-flat. We assume that in the guards are a conjunction of the atomic formulae of the form $\cur \le x$ or $\cur > x$ for $x \in X$. The situation that the guards are a conjunction of the atomic formulae of the form $\cur \ge x$  or $\cur < x$ can be considered similarly.
...
%
\item $\preceq'$ is obtained from $\preceq,g,\eta$ by the following (nondeterministic) procedure, 
\begin{enumerate}
\item initially, let $\preceq'_1 :=\preceq$,

\item for each conjunct $\cur \le x$ of $g$, let $\preceq'_1 := \preceq'_1 \cup \{(\cur, x), (x, \cur)\}$ or $\preceq'_1 := \preceq'_1 \cup \{(\cur, x)\}$, moreover, for each conjunct $\cur > x$ of $g$, let  $\preceq'_1 := \preceq'_1 \cup \{(x, \cur)\}$,

\item let $\preceq'_1$ be the reflexive and transitive closure of $\preceq'_1$,

\item let $\preceq':=\{(x,x) \mid x \in X\}$, and for each pair of variables $x_1, x_2 \in X$, 
\begin{itemize}
...
%
\item if $x_1 \in \dom(\eta)$ and $x_2 \in \dom(\eta)$ such that $\eta(x_1) = x'_1$, $\eta(x_2) = x'_2$, and $(x'_1, x'_2) \in \preceq'_1$  (where $x'_1, x'_2 \in X_s \cup \{\cur\}$), then let $\preceq':= \preceq' \cup \{(x_1, x_2)\}$.
\end{itemize}
%
\end{enumerate}
\end{itemize}

At first, from the construction, we know that $\Ss'$ is transition-enabled. It remains to show that $\Ss'$ is still generalized-flat. It is sufficient to show that for each state $q$ such that there is a self-loop around $q$ in $\Ss$, there does not exist  a cycle in $\Ss'$ that includes at least two distinct states $(q, \preceq_1)$ and $(q, \preceq_2)$.

To the contrary, suppose that there is a state $q$ in $\Ss$ such that there is a self-loop around $q$ in $\Ss$ and there exists a cycle in $\Ss'$ that includes two distinct states $(q, \preceq_1)$ and $(q, \preceq_2)$.
...
Let $S$ be the SCC of $\Ss$ that contains the state $q$. From the definition of monotone SNTs, each of $x_1,x_2$ computes the minimum or maximum value  in $S$. We distinguish between the following situations.
\begin{itemize}
\item Suppose $x_1$ computes the minimum value in $S$, $x_2$ computes the minimum value in $S$. Since $(x_1, x_2) \in \preceq_1$, we know that for each configuration $(q, \rho)$ reachable by traversing a path from the initial state to the state $(q, \preceq_1)$, we have $\rho \models x_1 \le x_2$. Since both $x_1$ and $x_2$ compute the minimum value in $S$, we know that for each configuration $(q, \rho')$ reachable from $(q, \rho)$ in $\Ss$, it holds that either $\rho' \models x_1 = x_2$ or $\rho' \models x_1 < x_2$. This implies that $(x_1, x_2) \in \preceq_2$, contradicting to the assumption.
%
\item Suppose $x_1$ computes the minimum value in $S$, $x_2$ computes the maximum value in $S$. Since $(x_1, x_2) \in \preceq_1$, we know that for each configuration $(q, \rho)$ reachable by traversing a path from the initial state to the state $(q, \preceq_1)$, we have $\rho \models x_1 \le x_2$. Since $x_1$ computes the minimum value and $x_2$ compute the maximum value in $S$, we know that for each configuration $(q, \rho')$ reachable from $(q, \rho)$ in $\Ss$, it holds that $\rho' \models x_1 \le x_2$. This implies that $(x_1, x_2) \in \preceq_2$, contradicting to the assumption.
%
\item Suppose $x_1$ computes the maximum value in $S$, $x_2$ computes the minimum value in $S$.  Since $(x_1, x_2) \in \preceq_1$, we know that for each configuration $(q, \rho)$ reachable by traversing a path from the initial state to the state $(q, \preceq_1)$, we have $\rho \models x_1 \le x_2$. Since $x_1$ computes the maximum value, $x_2$ compute the minimum value in $S$, we know that in $S$, the value $x_1$ is always increased and the value of $x_2$ is always decreased.  From $(x_1, x_2) \not \in \preceq_2$, we know that starting from the configuration $(q, \rho)$ and when following a path from $(q, \preceq_1)$ to $(q, \preceq_2)$, the value of $x_1$ becomes strictly greater than that of $x_2$, and this fact persists afterwards. Therefore, we have $(x_2, x_1) \in \preceq_2$ and $(x_1, x_2) \not \in \preceq_2$. Nevertheless, this implies that in each state $(q, \preceq')$ reachable from $(q, \preceq_2)$, it holds that $(x_1, x_2) \not \in \preceq'$, which contradicts to the fact that $(x_1, x_2) \in \preceq_1$.
%
\item Suppose $x_1$ computes the maximum value in $S$, $x_2$ computes the maximum value in $S$. Since $(x_1, x_2) \in \preceq_1$, we know that for each configuration $(q, \rho)$ reachable by traversing a path from the initial state to the state $(q, \preceq_1)$, we have $\rho \models x_1 \le x_2$. Since $x_1$ computes the maximum value and $x_2$ compute the maximum value in $S$, we know that for each configuration $(q, \rho')$ reachable from $(q, \rho)$ in $\Ss$, it holds that $\rho' \models x_1 = x_2$ or $\rho' \models x_1 < x_2$. This implies that $(x_1, x_2) \in \preceq_2$, contradicting to the assumption.
\end{itemize}
Consequently, in each of the four situations aforementioned, we always get a contradiction. We conclude that the assumption is false and $\Ss'$ is indeed generalized-flat.
\qed
\end{proof}
