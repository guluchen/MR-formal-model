...
%			$\rho'(z) =\rho(z)$ for $z\neq y$, $\rho'(y) = \eval{y+e}{\rho}$\\
			...
			$\rho'=\rho[\head(\tail(w))/\cur]$ if $\tail(w) \neq \epsilon$\\
...
Formally, the semantics of a program $p_0$ in the programming language can be defined as a graph in Figure~\ref{fig:semantics}. Each node in the graph is a triple $(p,w,\rho)$, where $p$ is a program, $w$ is the input data word, and $\rho$ is an valuation over $X^+\cup Y$ such that $\rho(\cur)=\head(w)$. 
Let $\rho_w$ be an assignment that $\rho_w(\cur)=\head(w)$ and $\rho_w(v)=\bot$ for $v\neq \cur$.
The set of initial nodes is $\{(p_0, w_0, \rho_{w_0})\mid w_0\mbox{ is an input data word}\}$.
We use $p(w)$ to denote the \emph{output} of a program $p$ with respect to a data word $w$. Then $p_0(w) =v$ if there exists a path from the initial node $(p_0, w, \rho_w)$ to some return node $(\mbox{ret }r,  \emptyset, \rho_r)$ and $
\eval{r}{\rho_r}=v$. Otherwise, $p_0(w)=\bot$. Since the program is deterministic, i.e., each input data word has at most one output, the semantics of $p_0$ is well-defined.
