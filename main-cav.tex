...
\item For the other contol variables as well as those newly introduced data values, their coefficients are from a bounded domain and can be dealt with easily.
\end{itemize}

\subsection{Analysis of a collection of cycles}

We will illustrate the argument for two simple cycles $C_1,C_2$ that share a unique common state.
...
Suppose that for $b=1,2$, the initial values of the $k$ control variables are $d^0_1,\dots,d^0_k$. Moreover, suppose that the $r_b$ data values $d^1_{1}, \dots, d^1_{r_b}$ are introduced when traversing the cycle for the first time, with one data value for each of $I_{1},\dots,I_{r_b}$. 
In addition, suppose that the initial values of $y_1,\dots, y_l$ are $o_1,\dots,o_l$. 

Then for $b=1,2$, the assignment function $\chi_{b,1}$ is of the following form,
\begin{itemize}
\item there is an injective mapping $\pi_b: \{1,\dots,k\} \rightarrow \{1,\dots, k+r_b\}$ such that for each $x_j \in X$, if $\pi_b(j) \le k$, then $\pi_b(j)=j$ and $\chi_{b,1}(x_j)=d^0_{j}$, otherwise, $\chi_{b,1}(x_j)=d^1_{\pi_b(j)-k}$,
% 
\item for each $y_j \in Y$, $\chi_{b,1}(y_j) = \alpha_{b,(j,0)} + \alpha_{b,(j,1)} o_j + \beta_{b,(j,1)} d^0_1 + \dots + \beta_{b,(j,k)} d^0_k + \gamma_{b,(j,1)} d^1_1 +\dots + \gamma_{b,(j,r_b)} d^1_{r_b}$ such that $\alpha_{b,(j,1)} \in \{0,+1,-1\}$.
\end{itemize}
...
We illustrate the argument for the situation that $J_{1,1} \cap J_{2,1} \neq \emptyset$, $J_{1,1} \setminus J_{2,1} \neq \emptyset$, and $J_{2,1} \setminus J_{1,1} \neq \emptyset$.

A \emph{cycle scheme} $\schm$ is a sequence $\schm=(1,\ell_1) (2, \ell_2 ) \dots (((m-1) \bmod 2)+1, \ell_m)$ such that $\ell_1,\dots, \ell_m \ge 1$. The number $m$ is called the length of the cycle scheme $\schm$.
...
In the following, we will illustrate the argument for Proposition~\ref{prop-mult-cycle} by considering a cycle scheme $(1,\ell_1)(2,\ell_2)$ such that $\ell_1,\ell_2 \ge 2$.
...
& & \sum \limits_{j' \in J_{1,1}} (\beta_{1,(j,j')}+\alpha_{1,(j,1)}\beta_{1,(j,j')} + \dots +\alpha^{\ell_1-1}_{1,(j,1)}  \beta_{1,(j,j')}) d^0_{j'} + \\
%
& & \sum \limits_{j' \in J_{1,2}} (\alpha^{\ell_1-1}_{1,(j,1)}\beta_{1,(j,j')}) d^0_{j'} +  \sum \limits_{j' \in J_{1,3}} (\alpha^{\ell_1-2}_{1,(j,1)}\beta_{1,(j, \pi^{-1}(j'+k))}+\alpha^{\ell_1-1}_{1,(j,1)}\gamma_{1,(j,j')}) d^1_{j'} + \\
%
...
%
& & \sum \limits_{j' \in J_{2,1}} (\beta_{2,(j,j')}+\alpha_{2,(j,1)}\beta_{2,(j,j')} + \dots +\alpha^{\ell_2-1}_{2,(j,1)}  \beta_{2,(j,j')}) d^0_{j'} + \\
%
& & \sum \limits_{j' \in J_{2,2}} (\alpha^{\ell_2-1}_{2,(j,1)}\beta_{2,(j,j')}) d^0_{j'} +  \sum \limits_{j' \in J_{2,3}} (\alpha^{\ell_2-2}_{2,(j,1)}\beta_{2,(j, \pi^{-1}(j'+k))}+\\
%
...
Intuitively, the coefficients of $\chi_{(1,\ell_1)(2,\ell_2)}(y_j)$ can be obtained those of $\chi_{(1,\ell_1)}(y_j)$ and $\chi_{(2,\ell_2)}(y_j)$ as follows.
\begin{itemize}
...
In general, for a cycle scheme $\schm=(1,\ell_1) (2, \ell_2 ) \dots (1, \ell_m)$ (suppose that $m$ is odd) and $j: 1 \le j \le l$,
\begin{itemize}
\item if $\alpha_{1,(j,1)}=1$ and $\alpha_{2,(j,1)}=1$, then some coefficients of $\chi_{\schm}(y_j)$ may be of the form 
\[
\begin{array}{l l}
& c_1 \ell_1  +   c_2 \ell_2  + c_1 \ell_3 + \dots + c_1 \ell_m   \\
= & (\ell_1 + \ell_3 + \dots + \ell_m) c_1  + (\ell_2 + \dots + \ell_{m-1})c_ 2 ,
\end{array}
\]
and all the other coefficients are from a bounded domain,
%
\item if $\alpha_{1,(j,1)}=1$ and $\alpha_{2,(j,1)}=-1$, then some coefficients of $\chi_{\schm}(y_j)$ may be of the form
\[
\begin{array}{l l}
& c_1 \ell_1  +   c_2  + c_1 \ell_3 + \dots +c_{m-1}+ c_m  \ell_m  \\
= & (\ell_1 + \ell_3 + \dots + \ell_m) c_1  + (c_2 + \dots + c_{m-1}),
\end{array}
\]
where for each $2 \le j_1 < j_2 \le m-1$, $c_{j_1}=c_{j_2}$ or $c_{j_1} = - c_{j_2}$.
\end{itemize}
