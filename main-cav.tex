...
Let $\interval{n} = \{ 1, 2, \ldots, n \}$. A \emph{permutation} on
...
$\pi_n(1)=2,\pi_2(2)=3,\ldots,\pi_2(n)=1, \pi_2(k)=k$, for $k>n$.
...
We use $\head(w)$, $\tail(w)$, and $|w|$ to denote the head, tail, and length of $w$, respectively.
...
\hspace{-0.7cm}
	...
	$(\ite{g}{s_1}{s_2};p, l, \rho) \longrightarrow (s_2;p, l, \rho)$& $\sub{g}{\rho} =\lfalse$\\
...
A \emph{streaming numerical transducer} (SNT) $\Ss$ is a tuple $(Q, X, Y, \delta, q_0, O)$ where 
\begin{itemize}
