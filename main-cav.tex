...
%
\item[Non-zero output reachability:] Given a SNT $\Ss$, decide whether $\Ss$ has a non-zero output, that is, whether there is a data word $w$ such that $\Ss(w)\notin \{\bot, 0\}$. 
\end{description}
The following propositions state that the commutativity problem can be reduced to the equivalence problem, which in turn can be reduced to the non-zero output reachability problem.
...
In this section, we present a decision procedure for the non-zero output reachability problem of SNTs.  In the rest of this section, we fix a (normalized) SNT $\Ss = (Q,X,Y,\delta,q_0,O)$ such that $X=\{ x_1,\dots, x_k\}$ and $Y = \{y_1,\dots,y_l\}$. Moreover, in the next three subsections, we assume that the guards of the transitions of $\Ss$ contain no comparisons with constants. In the last subsection, we will consider the more general situation. 
