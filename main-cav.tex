...
We introduce a concept of ``well-behaved'' for SNTs. Let $\Ss=(Q,X,Y,\delta,q_0,O)$ be an SNT. Then $\Ss$ is said to be \emph{well-behaved} if for each sequence of transitions of $\Ss$, say $q_0 \xrightarrow{(g_1,\eta_1)} q_1 \dots q_{n-1} \xrightarrow{g_n,\eta_n} q_n$, and each $i: 0 \le i \le n$, it holds that for each $y \in Y \cap \dom(\eta_i)$ and for each variable $z$ occurring in $\eta_i(y)$, we have $z \in \defval_{i-1}$, where $\defval_0,\dots,\defval_n$ are defined as follows,
\begin{itemize}
\item $\defval_0 =\emptyset$,
%
\item for $i: 0 < i \le n$, $\defval_i$  is constructed from $\defval_{i-1}$ as follows: 
\begin{itemize}
\item initially, let $\defval_i = \defval_{i-1} \cup (X \cap \dom(\eta_i))$,  
\item for each $y \in Y \cap \dom(\eta_i)$ such that for each variable $z$ occurring in $\eta_i(y)$, it holds that $z \in \defval_{i-1}$, add $y$ into $\defval_i$.
\end{itemize}
\end{itemize}

\zhilin{the definition of well-behavedness should be checked.}


From now on, we assume that all SNTs are well-behaved.
...
\end{itemize}
\end{itemize}
It is easy to see that $\Ss_1$ and $\Ss_2$ are  inequivalent iff there is a data word $w$ such that the output of $\Ss$ over $w$ is non-zero.
\end{proof}
