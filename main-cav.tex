...
Formally, the semantics of a program $p_0$ in the programming language can be defined as a graph in Figure~\ref{fig:semantics}. Each node in the graph is a triple $(p,l,\rho)$, where $p$ is a program, $l$ is the input list, and $\rho$ is an valuation over $X^+\cup Y$ such that $\rho(\cur)=\head(l)$. Let $R_l$ be the set of assignments $\{\rho_l \mid \rho_l(\cur)=\head(l)$, and $\rho_l(v) \in \intnum$ for $v\neq \cur\}$.
The set of initial nodes is $\{(p_0,l_0,\rho_{l_0})\mid l_0\mbox{ is an input list and }\rho_{l_0} \in R_l\}$.
...
The set of \emph{initial} configurations is $\{(q_0,\rho_0)\mid \forall z\in{X\cup Y}\rho_0(z)\in \intnum\}$. A sequence of configurations $(q_0,\rho_0)(q_1,\rho_1)\ldots(q_n,\rho_n)$ is
...
%%% we dont need the well defineness of SNT anymore!!
}
...
A SNT $\Ss=(Q,X,Y,\delta,q_0,O)$ is said to be normalized if the following two constraints are satisfied.
\begin{itemize}
	\item For each $(q,g,\eta,q') \in \delta$, if $\eta(x)=\cur$ for some $x \in X$, then the guard $g$ implies $\bigwedge_{x \in X} \cur \neq x$.  Intuitively, when the current data value is stored into some control variable, it is required that the data value is distinct from all the data values that have already been stored in the control variables.
	%
	\item For each $(q, g, \eta, q') \in \delta$, the guard $g$ is partitioned to the set of guards $\{g\wedge e \mid e\in\{\cur < c_{min}, \cur = c$ for $c_{min} \le c \le c_{max},\cur > c_{max}\}  \}$.
\end{itemize}
...
\end{proposition}
Intuitively, the construction used states to remember the equivalence relation between control variables and modify the assignments accordingly.
