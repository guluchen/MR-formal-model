...
$\Ss_{\max}=(\{q_0,q_1\}, \{\maxv\}, \emptyset, \delta, q_0, O)$, such that $\delta = \{(q_0, \ltrue, \maxv:=\cur, q_1), (q_1, \maxv < \cur, \maxv:=\cur,q_1), (q_1, \maxv \ge \cur, \emptyset, q_1)\}$, and $O(q_1)=\maxv$, where $\maxv:=\cur$ denotes the function assigning $\cur$ to $\maxv$.
\end{example}
...
We \emph{normalize} the SNTs to simlify the presentation of the decision procedure.
