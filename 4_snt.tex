...
\end{proposition}
Proposition~\ref{prop-snt-eqv-to-nzero} can be proved by a straightforward product construction. We would like to remark that the following property in the ``generalized flat'' constraint is essential for Proposition~\ref{prop-snt-eqv-to-nzero}: All the maximal paths in the SCC-graph of the transition graph of $\Ss$ have the same length. If this property is removed from the ``generalized flat'' constraint, then the SNT $\Ss_3$ obtained by product construction may not be generalized flat any more.
}
