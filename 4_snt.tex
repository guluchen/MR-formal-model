...
\end{itemize}
Intuitively, in the guards and transitions, the write-once control variables can occur in the same places as the special variable $\cur$.

 %Each transition is either of the form $(q, \cur \neq \triangleright \wedge g'_1, \eta, q')$ or $(q, \cur = \triangleright \wedge g'_2, \eta, q')$ such that $g'_2$ contains no occurrences of $\cur$. 
...
%, (3) {\it Control-variable-copyless:} for each transition $(q, g, \eta, q') \in \delta$ such that $x \in \dom(\eta)$, it holds that $\eta(x)=\cur$.
%
%\begin{itemize}
