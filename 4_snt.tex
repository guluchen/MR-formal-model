...
The semantics of an SNT $\Ss$  is defined as follows. A \emph{configuration} of $\Ss$ is a pair $(q,\rho)$, where $q \in Q$ and $\rho$ is a valuation of $X \cup Y$. An \emph{initial} configuration of $\Ss$ is $(q_0,\rho_0)$, where $\rho_0$ assigns arbitrary but mutually distinct values to the variables from $X\cup Y$.
...
In the end of this section, we state a property of SNTs, due to the fact that the guards are \emph{equality-free} (cf. the definition of guards in Section~\ref{sec:preliminaries}).

\begin{proposition}\label{prop-snt-distinct-value}
Let $\Ss$ be an SNT and $P$ be a path in $\Ss$. Then there is a data word $w$ such that 1) there is a run of $\Ss$ over $w$ which follows $P$, 2) no data values occur twice in $w$.
\end{proposition}
