...
\begin{itemize}
\item If $r^{\circled{C}}=1$ and $\cstl^{\circled{C}}_{j}=0$, then\medskip\\
\resizebox{0.9\hsize}{!}{$
...
\cstb^{\circled{C}}_{j, 1} \vard^{\circled{C , \ell}}_{1}.$}
%
...
\]
%
\item If $r^{\circled{C}}=1$ and $\cstl^{\circled{C}}_{j}=1$, then\medskip\\
\resizebox{0.95\hsize}{!}{$
\begin{array}{l }
